...
prothean and legion run the default NixOS kernel (`linuxPackages_latest`, set in `modules/common.nix`); citadel overrides it with `linuxPackages_testing` plus the official HDMI 2.1 VRR/ALLM patches for HDMI VRR (see Notes below). That RC-track kernel hangs on every shutdown/reboot — this was confirmed to be an upstream RC-track regression, not something the VRR patches cause — accepted anyway since the same hang happens on a plain RC kernel either way.
...
- **citadel:** boots directly into Steam/gamescope via Jovian, same as legion, with KDE Plasma 6 as the "Exit to Desktop" fallback (`hardware.openrgb`, `services.printing`, and `br`/`abnt2` keyboard kept from its old desktop-only setup for that fallback session). root/`/home`/`/nix` btrfs subvolumes tuned with `compress=zstd`, `noatime`, `space_cache=v2`, `discard=async` (+ weekly `fstrim`); extra `/GAMES` btrfs data drive. Runs `linuxPackages_testing` (`hosts/citadel/kernel-vrr.nix`, citadel only) with the official upstream HDMI 2.1 VRR/ALLM patch series applied — that series targets AMD's `amd-staging-drm-next` branch, whose already-split `amdgpu_dm.c` doesn't match mainline, so `patches/amdgpu-hdmi21-vrr-allm-testing-kernel/` carries the same logic hand-ported onto `linux_testing`'s monolithic `amdgpu_dm.c` (original series kept for reference in `patches/amdgpu-hdmi21-vrr-allm/`). That series missed the Linux 7.2 and 7.3 merge windows (confirmed 2026-08-10), now targeting Linux 7.4, expected end of 2026. Kernels on this RC-track line hang on every shutdown/reboot, but this was confirmed (2026-08-10) to be an upstream RC-track regression rather than something the VRR patches cause — `reboot=pci` and `amdgpu.runpm=0` didn't fix it, and nixpkgs' own vanilla `linuxPackages_testing` (unpatched) hangs identically. Kept anyway, since the hang happens either way and this way at least gets VRR.

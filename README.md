...
prothean and legion run the default NixOS kernel (`linuxPackages_latest`, set in `modules/common.nix`); citadel overrides it with `linuxPackages_testing` plus OpenGamingCollective's VRR/handheld patch for HDMI VRR (see Notes below) — still RC-track, so the reboot/shutdown hang below applies.

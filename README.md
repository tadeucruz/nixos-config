...
prothean and legion run the default NixOS kernel (`linuxPackages_latest`, set in `modules/common.nix`); citadel overrides it with a kernel built from AMD's `amd-staging-drm-next` branch for HDMI VRR (see Notes below). Kernels on that RC-track line hang on every shutdown/reboot — this was confirmed to be an upstream RC-track regression, not something the VRR patches cause — accepted anyway since the same hang happens on a plain RC kernel either way.

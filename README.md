...
prothean and legion run the default NixOS kernel (`linuxPackages_latest`, set in `modules/common.nix`); citadel overrides it with CachyOS's stable kernel for HDMI VRR (see Notes below) — a released kernel, not an RC, so it avoids the RC-track reboot/shutdown hang other approaches here hit.
...
- **citadel:** boots directly into Steam/gamescope via Jovian, same as legion, with KDE Plasma 6 as the "Exit to Desktop" fallback (`hardware.openrgb`, `services.printing`, and `br`/`abnt2` keyboard kept from its old desktop-only setup for that fallback session). root/`/home`/`/nix` btrfs subvolumes tuned with `compress=zstd`, `noatime`, `space_cache=v2`, `discard=async` (+ weekly `fstrim`); extra `/GAMES` btrfs data drive. Runs CachyOS's stable kernel (`nix-cachyos-kernel` input, `pkgs.cachyosKernels.linuxPackages-cachyos-latest`, citadel only) for HDMI 2.1 VRR/ALLM — CachyOS maintains a `7.1/hdmi` topic branch rebased forward each release, already merged into their regular stable build (confirmed 2026-08-13 by checking the release tarball source directly). Being a released kernel rather than an RC, it doesn't hit the reboot/shutdown hang that Linux's own 7.2-rc track has (confirmed 2026-08-10 to be an upstream RC-track regression). The official upstream AMD patch series for the same feature missed the Linux 7.2 and 7.3 merge windows, now targeting Linux 7.4 (end of 2026) — once it lands, citadel can drop CachyOS.

...
prothean and legion run the default NixOS kernel (`linuxPackages_latest`, set in `modules/common.nix`); citadel overrides it with the CachyOS RC kernel for HDMI VRR (see Notes below). That RC kernel hangs on every shutdown/reboot, but this was confirmed to be an upstream RC-track regression, not something the VRR backport causes — kept anyway since the same hang happens on a plain RC kernel either way.
...
- **citadel:** boots directly into Steam/gamescope via Jovian, same as legion, with KDE Plasma 6 as the "Exit to Desktop" fallback (`hardware.openrgb`, `services.printing`, and `br`/`abnt2` keyboard kept from its old desktop-only setup for that fallback session). root/`/home`/`/nix` btrfs subvolumes tuned with `compress=zstd`, `noatime`, `space_cache=v2`, `discard=async` (+ weekly `fstrim`); extra `/GAMES` btrfs data drive. Runs the CachyOS RC kernel (`nix-cachyos-kernel` input, citadel only) for an AMD-authored HDMI 2.1 VRR/FRL/ALLM patchset that missed the Linux 7.2 merge window and is backported onto CachyOS's 7.2-rc6 base (also missed 7.3 — confirmed 2026-08-10 — now targeting Linux 7.4, expected end of 2026). That kernel hangs on every shutdown/reboot, but this was confirmed (2026-08-10) to be an upstream RC-track regression rather than something the VRR backport causes — `reboot=pci` and `amdgpu.runpm=0` didn't fix it, and nixpkgs' own vanilla `linuxPackages_testing` (same 7.2-rc6 base) hangs identically. Kept the RC kernel anyway, since the hang happens either way and this way at least gets VRR.

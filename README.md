...
All three hosts run the default NixOS kernel (`linuxPackages_latest`, set in `modules/common.nix`). citadel previously overrode it with the CachyOS RC kernel for HDMI VRR, but that RC kernel's out-of-tree amdgpu DC patchset causes a hang on every shutdown/reboot (confirmed by bisecting it out) — reverted until fixed upstream (see Notes below).
...
- **citadel:** boots directly into Steam/gamescope via Jovian, same as legion, with KDE Plasma 6 as the "Exit to Desktop" fallback (`hardware.openrgb`, `services.printing`, and `br`/`abnt2` keyboard kept from its old desktop-only setup for that fallback session). root/`/home`/`/nix` btrfs subvolumes tuned with `compress=zstd`, `noatime`, `space_cache=v2`, `discard=async` (+ weekly `fstrim`); extra `/GAMES` btrfs data drive. Was running the CachyOS RC kernel (`nix-cachyos-kernel` input) for its out-of-tree HDMI 2.1 VRR/FRL patchset — mainline amdgpu still doesn't expose `vrr_capable` on HDMI as of Linux 7.2 (targeting 7.3) — but that kernel hangs on every shutdown/reboot (confirmed 2026-08-10: `reboot=pci` and `amdgpu.runpm=0` didn't fix it, removing the RC kernel did). Back on the default kernel until this is fixed upstream or in the patchset.

...
- **Kernel: OGC only on citadel, vanilla `linuxPackages_latest` on legion + prothean.** citadel's `modules/ogc-kernel.nix` starts from nixpkgs's `linux_7_2` and appends OGC's `monolithic.patch` (currently `v7.2-ogc4`) via `kernelPatches`; only the patch asset is `fetchurl`'d (never `fetchzip` of OGC's git archive — unpacked-tree hash varies by nix version). Buys citadel: **AMD HDMI 2.1 VRR/ALLM** + harmless handheld drivers. legion/prothean stay on stock 7.2.0 (everything from cache.nixos.org). Bumps via `scripts/update.sh ogc` (patch hash only; base stays `linux_7_2`), surfaced by `check-updates.yml`. **Known fragility:** `linux_7_2` floats with 7.2.x patch releases but the patch targets exactly 7.2.0 — a nixpkgs bump to 7.2.1+ may break the build; handled manually. A new OGC release on a different base (e.g. `v7.2.1-ogc2`) is applied anyway with a warning — validate the build before merging.

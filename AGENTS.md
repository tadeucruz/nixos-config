...
- **omega: NixOS stable, headless server** (`modules/server/`, no home-manager/flatpak — `mkServer` helper in `flake.nix`, tracks `nixpkgs-stable` = `nixos-26.05`). Migrated from Proxmox. `modules/server/` holds only generic infrastructure; workloads (VMs, LXCs, services) live in `modules/server/services/`:
  ...
  - **arr stack** (`modules/server/services/arr.nix`): qBittorrent + Prowlarr + Sonarr + Radarr + Bazarr as **native NixOS services** (decided over the original Podman Quadlet port from rannoch's `arrservice/docker-compose.yaml`). Media access via the `media` group (setgid dirs on `/mnt/media`); Prowlarr keeps its DynamicUser. qbittorrent's `openFirewall` only opens TCP — UDP 6881 is added manually. Migration: copy configs from `/mnt/docker/arrservice/configs` to each app's `dataDir` and re-point root folders/download client in the UI.
...
modules/
  ...
  server/
    ...
    services/                    # workloads: VMs, LXCs, and host services
      ...
      arr.nix                    # *arr stack (qBittorrent/Prowlarr/Sonarr/Radarr/Bazarr) — native NixOS services

...
# containers): created once on first boot via the download template, then
# autostarted by lxc.service on every boot. The container config is
# declarative (written from this repo). Requires the generic LXC module
# (modules/server/lxc.nix).
#
# TODO: swap the distro below if HermesAgent needs something else (Alpine etc.).
{
  config,
  pkgs,
  ...
  ...
  systemd.services.hermesagent-lxc = {
    ...
    script = ''
      if [ ! -d /var/lib/lxc/hermesagent ]; then
        ${pkgs.lxc}/bin/lxc-create -n hermesagent -t download -- -d debian -r bookworm -a amd64

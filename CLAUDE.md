...
- **legion: `linuxPackages_latest` (stable, no RC) + OpenGamingCollective's Lenovo WMI patch (`hosts/legion/kernel-ogc-lenovo.nix`, legion only).** Same OGC project as citadel's kernel, different release (`v7.1.8-ogc1`, pinned to a real stable 7.1.8 base — no RC-track reboot-hang risk). Adds `drivers/platform/x86/lenovo/*` (WMI battery charge limiting, CPU/GPU tunables) that upstream doesn't have yet. Controller support is separate, still via InputPlumber (`services.udev.packages = [ pkgs.inputplumber ]` for the `hid-lenovo-go` quirks) plus decky-loader + LegionGoRemapper.

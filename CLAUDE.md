...
- **citadel: CachyOS RC kernel (`nix-cachyos-kernel` input, citadel only).** Carries an AMD-authored HDMI 2.1 VRR/FRL/ALLM patchset (posted upstream, missed the 7.2 merge window, and — confirmed 2026-08-10 — also missed 7.3 after AMD's last DRM-Next feature pull for that cycle already went out; now targeting Linux 7.4, expected end of 2026) that CachyOS backported onto their 7.2-rc6 base. Current upstream series: `[PATCH v2 0/4] HDMI 2.1 VRR and ALLM support`, Fangzhi Zuo (AMD), https://lists.freedesktop.org/archives/amd-gfx/2026-August/150041.html (v1: https://lore.kernel.org/amd-gfx/20260730171754.704049-1-jerry.zuo@amd.com/). Uses the `release` branch (prebuilt + cached via the `attic.xuyh0120.win/lantian` substituter) to avoid local kernel compiles. **Known issue:** hangs on every shutdown/reboot (needs a hard power cycle) — confirmed (2026-08-10) this is *not* CachyOS-specific: nixpkgs' own vanilla `linuxPackages_testing` (same 7.2-rc6 base) hangs identically, so it's an upstream RC-track regression, not something the VRR backport introduces. Accepted tradeoff since the hang happens either way — keeping CachyOS for the VRR.

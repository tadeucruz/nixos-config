...
- **citadel: CachyOS RC kernel reverted (2026-08-10) — hangs on every shutdown/reboot.** Was using `nix-cachyos-kernel` (citadel only) for its out-of-tree HDMI 2.1 VRR/FRL patchset (still missing from mainline amdgpu — `vrr_capable` absent on the HDMI connector even on `linuxPackages_testing` 7.2.0-rc6; upstream targeting Linux 7.3, not guaranteed). Confirmed via console capture (Plymouth disabled) that every shutdown/reboot hangs right after `device_shutdown()` starts tearing down PCI devices — `reboot=pci` and `amdgpu.runpm=0` didn't fix it, but bisecting out the RC kernel did. citadel is back on the default kernel (`linuxPackages_latest`, same as the other two hosts) until this is fixed upstream or in the patchset.
